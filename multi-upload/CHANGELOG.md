...
- Reworked upload flow: dropped/picked folders are zipped in the browser (native `CompressionStream`,
  no libraries), uploaded as a single archive, extracted with `files/decompress` and the archive is
  deleted with `files/delete`, instead of creating folders and uploading files one by one.
